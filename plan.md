...
- **Connection registry (use a `map`, not a slice):** track open clients with a **mutex-protected map** for **O(1)** add/remove and a clear shutdown walk. Two good key choices:
  - `**map[net.Conn]struct{}`** — the accepted `**conn**` is the key; in `**handleConnection**`’s `**defer**`, `**delete(registry, conn)**` with the same value. No separate “lookup” when the client drops the TCP session: that goroutine already holds `**conn**`.
  - `**map[uint64]struct{}**` — assign `**id := atomic.AddUint64(&next, 1)**` (or similar) at accept, pass `**id**` into the handler, `**delete(registry, id)**` on exit. Handy if you prefer not to use an interface as a map key.
  On shutdown, **copy keys** (or snapshot conns) under the registry lock, **unlock**, then `**Close()`** each conn so you do not hold the lock during I/O. Idle handlers unblock from `**Scan()**` once their fd is closed.
...
1. **Add `Server` fields:** `shutdownOnce sync.Once`; `shutdownMutex sync.RWMutex` and `shuttingDown bool`; `inflightCmdWG sync.WaitGroup` for in-flight `**handleCommand`** calls; a **connection registry** as `**map[net.Conn]struct{}`** plus a `**sync.Mutex**` for add/remove/snapshot-before-close. Keep fields alongside the existing listener `mutex` / `namespaces` / `Port` / `MaxConnections`.
2. **Connection registry lifecycle:** when `serve` successfully takes a semaphore slot and starts `**go handleConnection(...)`**, **register** under the registry mutex (insert `**conn`** or `**id**`). In `**handleConnection**`’s `**defer**` path (inside or next to `**closeConnection**`), **unregister** with the **same** `**conn`** or `**id**` this goroutine already has — **no map search from another component** when the client closes their side. Each accepted client is removed **exactly once** when the handler exits.
...
5. **Inside the `Do` closure, match § Target shutdown sequence:** (a) under the listener `**mutex`**, swap `**s.listener**` to `nil` and `**Close()**` the listener; (b) `**shutdownMutex.Lock()**`, set `**shuttingDown = true**`, `**Unlock()**`; (c) `**inflightCmdWG.Wait()**`; (d) under the registry mutex, **snapshot** every `**net.Conn`** to close (for `**map[net.Conn]struct{}**`, copy the keys; for `**map[uint64]net.Conn**`, copy the values), **unlock**, then `**Close()`** each (usually **only** `**conn.Close()`** here — let each handler’s `**defer closeConnection**` still own `**<-connections**` so you do not double-receive on the semaphore); (e) `**s.namespaces.Shutdown()**`.
